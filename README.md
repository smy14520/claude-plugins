...
轻量 PRD-first 工作流：十个 skill（research / brainstorm / wayfinder / impl / check / review / wiki / init / handoff / architect）之间，进入要么用户发起，要么 agent 提议后用户同意，绝不单方进入。`prd.md` 的 slice checkbox 表示进度；`seed done` 重放显式测试/质量命令并写 `done-logs/`，显式 review-loop 可记录 task 级终态。没有 `evidence/`、task.json 或阶段状态机。
