...
轻量 PRD-first 工作流：九个 skill（research / brainstorm / wayfinder / impl / check / review / wiki / init / handoff）全部由用户主动触发、互不自动耦合。`prd.md` 的 slice checkbox 表示进度；`seed done` 重放显式测试/质量命令并写 `done-logs/`，显式 review-loop 可记录 task 级终态。没有 `evidence/`、task.json 或阶段状态机。
