...
### `/autolearn-sdd-kit:impl` 的状态语义

- `/autolearn-sdd-kit:impl <需求名>` 会继续使用 Claude Code 内置任务系统追踪本次会话内进度，同时把跨会话可见的执行快照写回 `.claude/tasks/<需求名>.tasks.md` 的固定 generated section（例如 `## Implementation Record`）。最终摘要推荐固定包含：`Requested mode`、`Actual mode`、`Status sync`、`Status file`。
