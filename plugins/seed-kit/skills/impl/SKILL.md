...
2. 按验证项的**义务（obligation）**逐条落证据；`[kind][surface]` 的 surface 不要改窄或改成别的面：
   - `[assert][...]`：`seed run-check <task> --slice S-NNN --obligation <id> -- <会失败的断言>` 真实执行，绑定到义务；失败就修代码再跑，不弱化断言。命令必须是会失败的断言——裸 curl/echo 这类烟雾命令对非 compliance 面会被 seed 硬挡，改成真正断言（测试套件 / `curl -sf` / 管道断言）。
   - `[judge][...]`：impl 不自评，开一个 **fresh-session 独立 agent**（生成者≠验证者）裁决。web-ui 视觉裁决走循环：**跑起来 → 截图 → 独立 agent 按高标开放 rubric 看真实产物打分 → 不到 bar 就改 → 再截图再裁**，直到过；rubric 压在整体设计质量与原创性、对照参考产品/设计语言、显式打低“通用 AI 味”，不是功能清单。过了用 `seed run-check ... --obligation <id> --verdict pass --trace "<依据>" --artifact "<截图路径>"` 落盘（`--artifact` 附实际看过的截图，helper 校验其存在）。verdict=fail 返工。
...
- **反复失败的根因分析**：同一 failing check 反复 3 次未过 → 停止盲改，进根因分析（复现 → 定位根因 → 假设 → 验证）再回来，不在失败循环里瞎改耗 context。
