...
2. 按验证项的**义务（obligation）**逐条落证据；`[kind][surface]` 的 surface 不要改窄或改成别的面：
   ...
   - `[judge][...]`：**impl 不自裁**（生成者≠验证者）。跑绿 assert 后**触发 review**，由 review 选择评分模式（Mode 1/2/3，见 review SKILL「多裁判对抗评分」），派独立裁判看真实 artifact、按项目 rubric 裁决。review 完成后落 evidence，再 `seed done`。
