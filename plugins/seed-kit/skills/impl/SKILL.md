...
4. **assert 绿后跑 `/seed-kit:review-loop`** 审到收敛（审代码+产物+propose-kill 3 票证伪，每个 slice 都跑，不只 `[judge]`）；未收敛按 review 返工清单修再 review。收敛后 `seed done <task> --slice S-NNN`（helper 按 `[kind]` + obligation 列缺口，按缺口补齐）。
