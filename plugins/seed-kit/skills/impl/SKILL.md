...
1. 读 `slices/S-NNN.md`，围绕其验收与 `## 交付面` 实现，按 AC 补测试；不弱化断言、不吞异常、不悄悄收窄 scope。
2. 按验证项的**类别和交付面**分别落证据；`[kind][surface]` 的 surface 不要改窄或改成别的面：
   ...
   - `[judge][...]`：impl 不自评。完成后**开一个 fresh-session 的独立 agent**（生成者≠验证者）按 rubric 裁决，再用 `seed run-check ... --judge "<项>" --verdict pass|fail --trace "<依据>"` 落盘。verdict=fail 就返工。
