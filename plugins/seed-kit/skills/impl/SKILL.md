...
2. 按验证项的**义务（obligation）**逐条落证据；`[kind][surface]` 的 surface 不要改窄或改成别的面：
   ...
   - `[judge][...]`：**impl 不自裁**（生成者≠验证者）。跑绿 assert 后交独立裁判看真实 artifact、按项目 rubric 裁决；质量/体验类优先用 `--rubric + --score-file` 让 helper 计算 verdict，legacy 语义裁决才用 `--verdict`。裁完再 `seed done`（gate 会因 judge 未裁或评分未达标而卡住，这是 per-slice 的预期闭环）。
