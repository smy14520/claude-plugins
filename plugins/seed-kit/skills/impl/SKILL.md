...
2. 按验证项的**义务（obligation）**逐条落证据；`[kind][surface]` 的 surface 不要改窄或改成别的面：
   ...
   - `[judge][...]`：**impl 不自裁**（生成者≠验证者）。跑绿 assert 后派一个 reviewer subagent（`Agent`，独立 context）裁该 judge——看 artifact、按 rubric，落盘走 `seed run-check --obligation <id>`（细节见 conventions）。裁完再 `seed done`（gate 会因 judge 未裁而卡，是 per-slice 闭环兜底）。
