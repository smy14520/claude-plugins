...
**默认 = 全量顺序模式**：用户没点名 slice 时，跑完所有未完成 slice；**一个 `seed-impl` agent 依次做所有 slice**（保持跨切片品质连贯）。全部 done 后进入「结束」节收尾。

**单 slice 模式**：用户点名了某 slice（如"做 S-003"）时，只做那一个。

1. `seed impl-state init <task>`（单 slice 模式加 `--slice S-NNN`）——落任务档案锚点（task_start_sha 写一次锁死）；档案已存在则跳过。然后 `seed status <task>`：checkbox + gate 失败计数 + 锚点 + 各 slice handoff/evidence 一屏可见。**续接任务**（档案里已有已完成 slice）先读其 handoff——前序隐性事实——不重做已完成 slice。
2. 项目记忆按需取用：wiki 存在时，agent 会在每个 slice 用 `seed wiki collect --query "<关键概念>"` 精准拉取相关 gotcha / cross_cut 页；编排层无需全量加载索引。
3. 通读 prd.md 全文（Goal、Acceptance Criteria、Out of Scope）——PRD 是合同。
...
**2. 检查 agent 产出**：
- agent 返回结构化 `slices` / `commands` / `issues` / `handoffs`，且实际测试和质量命令全绿 → 进步骤 3
- `handoffs` 逐条落盘：`seed handoff add <task> --slice S-NNN --note "<隐性事实>"`——中断/换终端续接的依据
...
**3. 主会话执行 durable gate**：对本次完成的每个 slice，使用 agent 返回的实际命令调用 `seed done <task> --slice {slice_id} --test "<命令>" [--quality "<命令>"]`。由 helper 重放命令；全 exit 0 才翻 checkbox。缺少显式测试命令时停止，不猜测、不代填。有产物证据时附 `--evidence-file <截图路径>`（可重复）/ `--evidence-url <验收实例 URL>`——指针写入任务档案，review/judge 不重跑即可查看。
...
- **gate 熔断**（`seed status` 显示某 slice gate 失败 ≥3 次）：停止报告用户；用户 `seed impl-state reset-attempts <task> --slice <id>` 清零后再继续

## 结束

所有 slice done 后，收尾自查——你自己做，不派 agent。**用全新的视角复审产出，重点找可能写错的地方**；整个自查闭环（发现问题→修→重验→落 marker）自动推进，不停下来请示——只有缺口拍板和 commit 归用户。
...
**2. 兑现对账**：读 `git diff` 与源码，逐条验收条目对账实现位置(file:line)与测试覆盖；**把你依赖的关键假设显式列出来**（接口语义、边界条件、并发前提）。发现漏条目、缺测试、机械问题直接修，改完重跑对应测试。
...
深度由你判断：简单任务快扫即过，复杂任务逐条细对。自查有抓不到的**盲点**（实现者写代码时带进去的假设）。**独立深审不在收尾自动派**——默认内联自查即收尾，不因“不放心”自动升级（升级常空转，实测有连派 2 次均无 finding）。需要独立视角时**由用户显式点名**：派 `seed-review` 在**干净 context** 审某段，或 `/seed-kit:review-loop` 审整个 task。
...
完成后汇总改动范围（有锚点时整体 diff 用 `git diff <task_start_sha>`，SHA 从 `seed status` 读）、证据位置与关键假设清单。
