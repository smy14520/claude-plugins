...
**默认 = 全量顺序模式**：用户没点名 slice 时，跑完所有未完成 slice；**每个 slice 必须 impl → 跑 `/seed-kit:review-loop`；converged 才 `seed done`、然后进下一个；非 converged = 没搞定，停下报告用户并等介入、不自动进下一个**。"跑完全部"指每个都含 review 地做完，**不是跳过 review 赶进度**——review 是 slice 之间的硬门槛。
**单 slice 模式**：用户点名了某 slice（请求里出现 S-NNN，如"做 S-003"）时，只做那一个，不自动推进。

1. `seed status <task>` —— 断点续作入口：结构错误先停下报告；否则从 `next` 指向的 slice 继续（全量模式下逐个推进）。
...
5. 提示 commit 本 slice 的改动（agent 不自动 commit）。**自动化 / 连续模式**下：口头提示即可，不要停下来等确认，继续下一个 slice。
...
所有 slice 完成后：汇总本次改动范围与证据位置。review 已在每个 slice 执行，无需额外触发。（**自动化 / 连续模式**：slice 勾选成功即完成——不必再跑 `seed status` 自证、不必停下来等 commit 确认，汇总即可。）
