...
1. 读 `slices/S-NNN.md`，围绕其验收实现，按 AC 补测试；不弱化断言、不吞异常、不悄悄收窄 scope。
2. 按验证项的**类别**分别落证据：
   ...
   - `[human]`：提示用户/stakeholder 签收，用 `seed run-check ... --human "<项>" --note "<结论>" --by "<签收人>"` 记录。
3. 不允许把可 assert 的检查降级成 judge/human 充数。
