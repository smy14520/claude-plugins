...
`next-action` 返回 `integration_review`（全部 slice done）→ 做**集成 review**（主会话自己做，不派独立 review agent），diff 范围直接用返回的 `diff_range`。集成 review 补 per-slice review 看不到的盲区——跨 slice 集成问题。
...
收尾：
- 落 task 级 marker：收敛 → `seed review-mark <task> --verdict converged --note "integration-review"`；2 轮仍有 finding → `seed review-mark <task> --verdict rounds-exhausted --note "integration-review escalated"`。
- **报告改动范围**（给人审用）：
  ```
