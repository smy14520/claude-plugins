...
3. 每个 slice 写一个 `slices/S-NNN.md`（标题与索引行一致）：
   ...
   - **验证项设计规则见 conventions**（覆盖、kind、烟雾、AC 维度）——这里只写 slice 形状，不重述。仅记两条：human 覆盖可断言交付面是设计气味；**设验证面前先读项目的 `.claude/rules/`（测试纪律）与 `DESIGN.md` / 质量基线（rubric、bar、品味）**，项目无则用通用地板。
