...
3. 每个 slice 写一个 `slices/S-NNN.md`（标题与索引行一致）：`## 验收` 写对应 AC 与细节；`## 验证` 至少一条验证项，按"谁判定它对"归入三类（见 conventions）：
   ...
   - **judge**：难以机械断言的语义/UI 行为用 `[judge]`，注明 rubric 位置；由独立 agent 在 fresh session 裁决。
   - **human**：真正的涉众签收。**testability gate**：一个 slice 如果只能 `[human]` 验证，是设计气味——显式标出来并写明为什么无法 assert/judge，让用户判断是接受还是改设计。
