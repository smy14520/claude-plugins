---
name: brainstorm
description: "把模糊想法收敛成可执行的 PRD。访谈式提问——用 AskUserQuestion 一次问一个高价值问题（推荐项置首、用户可改/另填），终点是 .arbor/tasks/<task>/prd.md：可证伪 AC + 有序 Slices（每 slice 声明验证命令）。"
---
# Brainstorm — 需求收敛访谈
...
- **用 AskUserQuestion 工具提问，不要用散文**：`questions` 只放一条（一次一题），2–4 个选项，推荐项置首、label 带"(推荐)"，取舍写进各选项 description；开放题也走选项——用户总可选 Other 自由输入，不会被选项框死。
...
3. 每个 slice 写一个 `slices/S-NNN.md`（标题与索引行一致）：
   - `## 交付面` 声明本 slice 实际交付的面（参考词汇表 backend-domain、api、web-ui、e2e、compliance、infra，项目可扩展，定义见 verification）。
   - `## 验收` 写对应 AC 与细节。
   ...
   - **验证项设计规则见 verification**（覆盖、kind、烟雾、AC 维度）——这里只写 slice 形状，不重述。仅记两条：human 覆盖可断言交付面是设计气味；**设验证面前先读项目的 `.claude/rules/`（测试纪律）与 `DESIGN.md` / 质量基线（rubric、bar、品味）**，项目无则用通用地板。
   - 有可感面时，质量基线 + `DESIGN.md` 定体验意图；需要 gate 的质量项写成 `[judge]` obligation，后续用 scoring gate 落证据。
