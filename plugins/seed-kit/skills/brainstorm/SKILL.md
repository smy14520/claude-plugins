...
3. 每个 slice 写一个 `slices/S-NNN.md`（标题与索引行一致）：`## 交付面` 声明本 slice 实际交付的面（闭集 backend-domain / api / web-ui / e2e / compliance / infra，定义见 conventions）；`## 验收` 写对应 AC 与细节；`## 验证面` 至少一条验证项，每条 `[kind][surface] target`。**验证项设计规则见 conventions**（覆盖 / kind / 烟雾 / AC 维度）——这里只写 slice 形状、不重述；仅记一条：human 不能替代可断言交付面。有可感面时，质量基线 + 项目 `DESIGN.md` 定体验意图。
