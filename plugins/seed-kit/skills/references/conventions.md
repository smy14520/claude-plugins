...
- 七个 skill（research、brainstorm、impl、impl-agent、check、review、wiki）全部由用户主动触发，互不自动切换阶段——skill 之间不自动推进（例外：check 是 impl 收尾的默认步骤，用户点名 impl 即选择该收尾语义）。
...
- **Skill**（用户主动触发，**无** `seed-` 前缀）：`seed-kit:brainstorm` / `seed-kit:impl` / `seed-kit:impl-agent` / `seed-kit:check` / `seed-kit:review` / `seed-kit:research` / `seed-kit:wiki`
...
seed review-mark <task> --verdict <reason> [--round N] [--depth single|full]  # 落终态 marker（depth=single 默认单 agent 审查 / full=5 agent review-loop）
