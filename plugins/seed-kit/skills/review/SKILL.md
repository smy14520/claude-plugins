...
- 专查**措辞红旗与逐条对账**：`should/seems/大概/基本上/应该` 这类掩饰不确定的措辞；acceptance criteria **逐条对账**（声明的每条 vs 真验证的每条）；歧义与自相矛盾。
...
- 专查**交付面冒充**：`## 交付面` 声明的每个面是否都有真实实现与证据支撑；验证项不能跨面冒充（声明 web-ui 却只给标 backend-domain 的后端测试）；可断言的面别用 human 充数（该面该什么 kind 按项目 `.claude/rules`）。
...
- **整体审计时**：review 触发后按下方「选模式」执行；若发现 impl 漏裁的 `[judge]`，兜底裁掉——看真实产物、按 rubric，用 `seed run-check` 落盘 verdict（完整命令见 conventions；judge 靠 `--verdict pass|fail` 进模式、必带 `--artifact`）。verdict=fail 记返工清单。

### 体验质量类 judge：评分循环

不裸 pass/fail，走「看真实产物 → 按项目 rubric 各维度打分 + critique → 不到 bar 记返工 → impl 改 → 再看再裁」直到过。落盘用 `--grade`（各维度分）+ `--trace`（依据 + 不到 bar 的点）+ `--artifact`（看过的截图）。rubric 的维度、bar、参考来自项目（`DESIGN.md` / `.claude/rules/`），插件不硬编码；rubric 压在整体设计质量与原创性、打低「通用 AI 味」，不是功能清单。grade 不进 gate（gate 只看 verdict），留给质量看板/复核。
...
| 多个不重叠 lens（安全+性能+体验）**且要互相证伪** | **agent team** | `TeamCreate` 开 reviewer teammate，peer 互相挑战，`TaskCompleted` exit 2 做硬门 |
| judge **多角度独立评分后聚合** / 多 slice / codebase-wide sweep | **workflow** | `Workflow` 脚本 fan-out 多裁判，投票/filter 聚合 |

**重型（team/workflow）知会一句再起**：给一句话理由 + 推荐（如"本任务有 3 个对抗 lens，建议开 team"），subagent 全自动。team/workflow token 重，符合"AI recommends, user decides"，默认不静默起。
