...
- **P0-3 第一批:认证未通过,candidate 不合并(2026-07-26)。** judge 原判 improved/tradeoff,被 5-agent 盲评审计 + trajectory 取证推翻(详见 evals-v2 两份 CONCLUSION.md):
  - 成本 -43%/-52% 的主体(~92%)是 trials=1 下的**编排运行方差**(后台派发+sleep 轮询+task 簿记),两版 SKILL 都没规定这些行为;被删条款可归因节省 <4%。
  - 可靠性出现与清剪方向精确对应的疑似退化:treatment 唯一 major(docstring 契约违约)+ 全文件 1 个 raise vs control 12 个——**恰是被删"输入校验"条款的方向**。
  - **关键机制修正:自审 checklist 的真实作用是"实现期预塑形"而非"实现后自查"**——自查环节 5 次全走过场(零触发),但 prompt 关键词让实现期长出防御层。删它省不了钱(<0.1% token)却可能丢防御性。清剪判据需增加:该条款是否含实现期塑形词?"保姆条款=纯噪音"假设对这类条款不成立。
