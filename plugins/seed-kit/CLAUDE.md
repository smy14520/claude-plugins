...
seed-kit 只提供**机制**（栈无关）：验收义务 obligation（形如 `[kind][surface] <id>: <行为>`，证据以 obligation_id 绑定）、验证三 kind（assert / judge / human）、交付面覆盖校验（面名自由、非封闭）、正确性 gate、`seed` CLI。当前 gate 仍是 verdict=pass/fail；review 可附 `--grade` 作质量反馈（grade 不进 gate）。多裁判并行评分循环（fan-out）尚未实装。
