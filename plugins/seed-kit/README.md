...
- **judge** — 由独立 agent 裁决（生成者≠验证者，详见 conventions），看渲染后的真实产物按 AC rubric 裁决，`--artifact` 附看过的截图（pass 必须；helper 校验存在）。gate = verdict=pass。
...
- gate 只保证“声明命令被执行并落盘”，**不保证语义正确**——语义可信度由交付面结构约束、三类词汇、烟雾嗅探、独立 judge、review 共同保证。
