...
每个 slice 显式声明 `## 交付面`（backend-domain / api / web-ui / e2e / compliance / infra），验证项写成验收义务 `[kind][surface] <obligation-id>: <可观测行为>`：

- **surface** — 覆盖哪个交付面；helper 做确定性结构约束，防止后端测试冒充 Web/UI/E2E 覆盖。
- **assert** — 会失败的断言命令（测试套件 / 契约回放 / Playwright spec）。gate = exit 0。
- **judge** — 独立 agent（fresh session，生成者≠验证者）看渲染后的真实产物按 AC rubric 裁决，`--artifact` 附看过的截图（helper 校验存在）。gate = verdict=pass。
...
- `run-check --obligation <id>` 把证据绑到 slice 声明的义务（命令不写在 slice，由 impl 绑定）；**烟雾命令**（裸 `curl`/`echo` 这类“跑过就算过”）对非 compliance 交付面硬挡、compliance 面警告——它们只证可达不证语义，也不能覆盖交付面。
...
- `seed done` / 绿看板 = **正确且不回归，不代表体验质量达标**；好不好用这类不可验证质量靠“测试当地板 + 独立高标 judge 看真实产物在环 + 生成自由”把关，不由 gate 保证（见 [DESIGN.md](DESIGN.md) 的两种失败模式）。
