...
seed run-check <task> --slice S-NNN \
  ...
  --artifact "<看过的截图>" [--grade "..." --by "..."]   # [judge] 独立 agent 裁决后落盘（pass 必须附 --artifact）
seed run-check <task> --slice S-NNN \
...
- **judge** — 独立 agent（agent team 的 reviewer 或 subagent，生成者≠验证者）看渲染后的真实产物按 AC rubric 裁决，`--artifact` 附看过的截图（pass 必须；helper 校验存在）。gate = verdict=pass。
