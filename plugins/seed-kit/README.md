...
```
.seed/tasks/<task>/
├── prd.md        # 需求 source of truth；## Slices 的 checkbox = 唯一进度状态
├── review.md     # review 追加记录
├── evidence/     # seed run-check 落盘：S-NNN/<seq>-<kind>.json + .log
└── notes/        # impl 过程备注（可选）
```

断点续作 = `seed status <task>` + git log。代码就是进度。
...
seed run-check <task> --slice S-NNN -- <命令>     # 真实执行 PRD 声明的命令，落盘 exit_code + 输出
...
- `run-check` 的命令必须与 PRD `- 验证:` 声明完全一致——不接受替代或弱化命令。
