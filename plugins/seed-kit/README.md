...
- **judge** — 由独立 agent 裁决（生成者≠验证者，详见 conventions），看真实产物按项目 rubric 裁决。legacy gate = `verdict=pass`；scoring gate = helper 读取 `--rubric` + `--score-file` 并计算 verdict。`--artifact` 指向看过的产物，helper 校验存在。
