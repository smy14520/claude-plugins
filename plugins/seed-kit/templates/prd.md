...
<!-- 每条 AC 可证伪：Given/When/Then，至少覆盖一条失败路径。
     AC 可带编号（AC-1/AC-2…），brainstorm 会把它带进 slices/S-NNN.md 验证面的 obligation-id
     作 review 对账线索（helper 不解析 AC 全集；覆盖靠交付面校验 + review 对账）。
     写需求时标注每条用哪类验证兑现（assert / judge / human）、覆盖哪个交付面
     （backend-domain / api / web-ui / e2e / compliance / infra）——这决定 slices/S-NNN.md 的形状。 -->
