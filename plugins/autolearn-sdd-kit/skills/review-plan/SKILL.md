...
如果文件中存在 `## Implementation Record` / `autolearn:impl-status` 这类 generated execution section，把它视为执行快照而不是计划正文；审查时忽略该区块，只检查原始 Task 定义、depends_on、files、steps、acceptance。
