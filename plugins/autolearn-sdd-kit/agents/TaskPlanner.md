...
2. **切片完整性**：检查每个 Task 是否围绕一个最小完整交付切片；每个 Task 应对应一个可独立验证的行为增量，而不是某一类文件或技术层的归类集合。
3. **共享产物判定**：只有当某部分本身是共享产物、能独立验证、并且明确作为其它 Task 的前置依赖时，才应单独拆成 Task。
4. **files 完整性**：所有 step 中创建、修改、验证的文件，尤其新建测试文件，都必须出现在对应 Task 的 `files` 列表里。
5. **依赖方向 / Producer-Consumer**：如果 Task B 负责生产前置产物，任何先消费这些产物的 Task 都不能排在它前面。
