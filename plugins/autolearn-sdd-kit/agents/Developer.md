...
4. **完成后立即返回，不要挂起等待**
5. **`.claude/tasks/<需求名>.tasks.md` 的原 Task 正文是 spec，不要改写正文追踪进度；只允许维护固定 generated execution section**
...
2. 解析所有 Task 及其依赖关系、`files`、acceptance
...
4. 在 `./.claude/tasks/<需求名>.tasks.md` 中维护固定执行状态区块：
   - 区块标题建议为 `## Implementation Record`
   - 使用 marker：
     - `<!-- autolearn:impl-status:start -->`
     - `<!-- autolearn:impl-status:end -->`
   - 如果区块不存在，插入；如果已存在，只覆盖 marker 之间内容
   - 仅记录执行快照，不修改原 Task 正文、acceptance、depends_on、steps
5. 初始化状态区块至少包含：
   - Overall status: `in_progress`
   - Requested mode: `sequential` / `parallel_requested`
   - Actual mode: 初始可写 `pending`
   - 每个 top-level Task 的运行态摘要（pending / blocked / in_progress / completed）
...
2. 同步刷新 execution section，把当前 Task 记为 `in_progress`
3. 读取 `role` 和 `stack` 标签，调整实现策略
4. 逐个执行 step（原子动作）：
   - 简单修改：用 Edit 工具精确修改
   - 新建文件：用 Write 工具创建
   - 运行验证：用 Bash 执行命令
...
7. 再次刷新 execution section：
   - 当前 Task 记为 `completed`
   - 记录已完成 Task 列表
   - 记录本 Task 实际修改文件
8. 进入下一个 Task

### 并行执行（用户要求时，使用 subagent）

当用户要求并行时：

1. `--parallel` 指的是 **subagent fan-out**，不是 Team 模式；默认不要创建 Team，也不要使用 `TeamCreate`
2. 读取所有 Task 的 `depends_on` 和 `files` 字段
3. 只有同时满足以下条件的 top-level Task 才允许并行：
   - `depends_on: []` 或前置任务已完成
   - `files` 无重叠且无明显冲突
4. 如果存在安全可并行组，在**同一个消息中**发出多个 Agent 工具调用，为每个独立 Task 启动一个 fresh subagent：
...
5. 有依赖的 Task 等前置 subagent 返回后再执行
6. subagent 之间不要操作同一文件
7. 如果不存在安全可并行组，**必须显式降级为顺序执行**，并在 execution section 与最终摘要里写明原因，例如：
   - `depends_on` 串行化
   - `files` 冲突/边界不清晰
8. 无论最终是否真正并行，最终摘要都必须说明：
   - Requested mode
   - Parallel strategy（真正 fan-out 时写 `subagents`；未使用并行编排时写 `none`）
   - Actual mode（`sequential` / `parallel_used` / `parallel_fallback`）
   - 若 fallback，给出一句具体原因

## Execution section 写入约束

- 只允许维护固定 generated section，不要改写原 Task 标题后的 `⏳ / 🔄 / ✅`、不要改原 step checkbox
- 建议 section 内容包含：
  - Last run
  - Requested mode
  - Parallel strategy
  ...
  - Status sync result
  - Overall status
  - 按 Task 列出的 runtime status
  - Completed tasks
  - Modified files
  - Blockers（如有）
- 对同一需求重复执行时，覆盖旧 section，不要无限追加日志
...
**必须使用 TaskCreate/TaskUpdate 作为会话内真源；同时把跨会话可见的执行快照同步到 `.tasks.md` 的 generated execution section。**
...
Status file: <如有则填 ./.claude/tasks/<需求名>.tasks.md>
