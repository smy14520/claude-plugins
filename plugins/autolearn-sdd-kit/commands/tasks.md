...
4. 优先按“完整行为切片”拆任务，而不是按 UI / i18n / 测试 / 迁移 / schema / 配置这类技术层机械拆分；只有当某部分本身是共享资产、能独立验证、并且明确作为其它 Task 的前置产物时，才单独拆成 Task。
5. 如果某个 Task 负责生产共享产物（如类型、schema、迁移、文案、共享 helper、测试基座、IPC 契约），任何消费这些产物的 Task 都必须依赖它；不要出现 producer 在后、consumer 在前的依赖倒置。
