...
3. 执行 `sdd-arbor derive-required-checks <package>`，把 slice Verification 固化成 required checks；后续只能逐项满足或记录阻塞，不能自行省略。
4. 读 `task.json.slices` 判断进度：
...
5. 若 `review_result.state` 是 `NEEDS_REWORK`（CLI 参数 `needs_rework`），读问题清单针对性修复，不从头执行 slice。
6. 状态非 doing 时 `sdd-arbor set-status <package> --state doing --actor impl --note "开始执行"`。
...
3. 对该 slice 的 required checks 逐项交账：
   - 有命令 / 可自动化的 required check 用 `sdd-arbor run-check <package> --required-check <req_id> -- <command>`。
   - 只能人工观察的 required check 用 `sdd-arbor record-check <package> --required-check <req_id> --status passed --evidence "<观察证据>"`。
   - 不能执行的 required check 必须用 `record-check --status blocked|not_run --reason "<原因>" --evidence "<阻塞证据>"`；不能把 runnable check 当 manual/not_run 省略。
...
全部 slice 完成后检查 required checks：

| 层 | 内容 | 证据要求 |
|---|---|---|
| A. Build | 项目能编译 / 构建无报错 | `run-check` exit_code=0 |
| B. Test | 按 Testing strategy 的测试全部通过 | `run-check` exit_code=0 |
| C. Functional | 按 Technical Framing 的 stack / runtime 启动并验证核心路径 | browser/api/docker/manual evidence |

Build + Test 通过不等于 DONE。Functional 是必须的。写了测试文件、读代码觉得会过、或 reviewer 口头判断都不是 test evidence。
...
`--acceptance` / `--check` / `--concern` 可重复。`--command` 仅为 legacy note，不是 verification evidence；DONE / DONE_WITH_CONCERNS 必须引用 check id。
