...
5. 按 `red signal → change → green command/result` 记录证据；最终仍要满足 task acceptance，并用 `record-impl-result` 记录结果。
...
- 视觉、动画、拖拽手感和响应式细节需要浏览器验证或 visual regression 补足；UI TDD 通过不替代 browser golden path。

## External boundaries

- 只在系统边界 fake / mock：第三方 API、支付、邮件、LLM、外部 webhook、时间、随机数、网络。
- 不 mock 内部业务逻辑来制造通过。
- provider adapter 用 fake server、sandbox 或 recorded fixture 验证 request/response mapping、错误码、timeout、重试和 malformed response；默认不打真实第三方，除非 task 明确 sandbox/live acceptance。
