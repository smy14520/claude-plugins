...
- 完成标志:现有 users / orders / products 查询自动按 tenant_id 过滤;跨租户查询返回空
...
- 完成标志:`mytool init <project>` 在空目录创建 manifest 并以退出码 0 结束
- 代码锚点:cmd/init [new]
- 测试:init_test.go 覆盖 happy path / 已存在目录 / 权限错误

### S-003: ML feature contract

- 完成标志:`python -m features.build --date 2026-05-01` 写出 feature_v1.parquet 且 schema 校验通过
- 数据 / schema:feature_v1 schema [new] artifacts/feature-schema.json
- 代码锚点:features/build.py [new]
- 测试:feature_build_test.py 覆盖 schema 校验 / 缺失输入 / 重跑幂等
```

S-001 涉及数据、代码、测试三字段都写;S-002 没动数据层,所以省 `数据 / schema`;S-003 涉及文件格式 / 模型输出,所以写 `数据 / schema` 并指向 artifact。字段由 slice 实际涉及范围决定,不由项目类型决定。
