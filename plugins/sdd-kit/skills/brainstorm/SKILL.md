...
2. 读 PRD 当前状态；如果用户的回答让新的代码区域变得相关，读取该区域再继续。
3. 沿用户场景逐个展开——当前场景的触发、行为、成功判定、退化路径和交叉影响是否都有行为级精度？
4. 有未达到行为级精度的场景 → 按 Hard rule 2 用 `AskUserQuestion` 问一个最高价值问题，**停下等用户**；所有场景都已收敛 → 进入 Finalize。
...
PRD 必须收敛承重技术边界，避免 impl 在关键架构问题上自由发挥。覆盖 stack / auth / implementation shape / ownership / source of truth / data / admin / integration / testing / migration 档位。Testing strategy 档位用 `AskUserQuestion` 让用户选。详见 [`references/technical-framing.md`](references/technical-framing.md)。
