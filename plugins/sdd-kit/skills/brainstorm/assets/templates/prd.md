...
<!-- 
PRD 必须收敛承重技术边界，避免 impl 在关键架构问题上自由发挥。
brainstorm 根据项目画像（web / cli / lib / data）选择覆盖项；详见 references/technical-framing.md。

通用项（所有项目必需）：
- Existing stack / framework
- Source of truth / 事实源
- Implementation Shape / 实现形态
- Testing strategy

画像特定项按实际需要从 references/technical-framing.md 中选取，不相关的省略。
新项目：记录技术选型和架构边界。
存量项目：基于代码分析，记录现有结构和变更方案。
-->

- Project profile: <web / cli / lib / data / game，可叠加；不匹配时写领域名>
- Existing stack / framework:
- Source of truth / 事实源:
- Implementation Shape / 实现形态:
  - 参考形态: <贴近哪些 repo / framework 既有代码形态>
  - 组装方式: <功能如何由各层组合完成>
  - 避免形态: <哪些写法会造成大文件、重复逻辑、绕过事实源或难以 review>
- Testing strategy:

<!-- 以下为画像特定项，按 references/technical-framing.md 中当前画像对应的列表选取，不相关的整条删除 -->
...
<!-- Brainstorm 定稿前写好。每个 slice 是一个行为级 checkpoint——对应一个用户可观察行为或系统 invariant，能用一句 Given/When/Then 描述。PRD 里只保留标题 + 完成标志作为索引；详细的 Acceptance（G/W/T）、Approach、Verification 写在 slices/S-NNN.md task 文件中。完整规则见 references/slices.md。 -->

### S-001: <walking skeleton 或第一个独立可验证的行为>
...
- [ ] Technical Framing 覆盖通用项 + 当前画像特定项的承重边界；不相关画像项已省略（不写 N/A）
