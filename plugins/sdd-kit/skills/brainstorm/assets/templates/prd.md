...
常见 artifact：
- `artifacts/data-model.sql`：草案级 schema contract，非生产 migration。
- `artifacts/integration-contract.md`：第三方协议边界、payload、验签/鉴权规则。
- `artifacts/api-contract.md`：关键 API contract。
-->
...
<!-- 
新项目：列出需要的数据实体和关键属性；若字段/关系较多，放入 artifacts/data-model.sql 并在这里引用。
存量项目：列出涉及的现有表（标注 [existing]）和新建表（标注 [new]）及关键变更；完整 SQL / schema contract 放入 artifacts/ 或引用现有 migration/model source。
-->
- <涉及的状态、表、缓存、队列、幂等键等>
