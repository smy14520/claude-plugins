...
```text
parallel-step
→ execute returned safe_actions
→ dispatch returned worker assignments
→ handle worker reports with finish-worker / reconcile-package / upsert-contract
→ repeat until parallel-step returns complete or true blocker
```
...
如果 lead 已知 runtime worker 状态，把观察结果传给 helper：`--live-worker <name>` 可重复；确认没有 live Team worker 时传 `--no-live-workers`。

`parallel-step` 是 lead 的确定性 action-plan 入口：它组合 schedule / self-healing / dispatch，返回 `safe_actions`、`dispatch` 和 `stop_reasons`。Skill 执行 action plan，不手工重建调度状态机。
...
```text
parallel-step            # 组合 schedule / self-healing / dispatch，返回 lead action plan
...
→ run parallel-step
→ execute safe_actions in order
→ claim/dispatch returned assignments
→ route WAITING_INPUT / BLOCKER / CONTRACT_REQUEST through helpers
...
→ rerun parallel-step
→ stop only on complete or true blocker
```

关键规则：

- `parallel-step` 是 lead action-plan 事实源；Team idle 不是 durable truth，但 live worker 观察必须通过 `--live-worker` / `--no-live-workers` 结构化传给 helper。
- `map-check` / `parallel-schedule` 是底层状态事实源，不替代 action plan。
...
3. 运行 `sdd-arbor parallel-step <initiative> --max-parallel 5 --json`。
4. 如需创建 Team：使用 step 返回的 `team_name` 创建 `TeamCreate(team_name="arbor-<initiative>")`；不要创建 nested orchestrator teammate。
5. 执行 step 返回的 `safe_actions`；对 `dispatch[]`：先执行 action.command claim package，再确保 package worktree/branch，然后 spawn worker。
6. `worktree_root` 默认 project sibling：`../arbor-worktrees/<project-name>`。`.arbor` 中只持久记录 portable ref，不写本机 absolute path。

## Dispatch

对每个 `parallel-step.dispatch[]` action：

1. 执行 action.command 完成 pre-claim。
