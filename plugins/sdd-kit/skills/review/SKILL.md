...
impl 已经把审计材料结构化到 `task.json.impl_result`，review 必须用这些字段对账，不能只读自然语言：

| 字段 | 用途 |
|---|---|
| `acceptance` | 对照 PRD 每个 slice 完成标志，逐条找代码 / 测试证据 |
| `commands` | 对照 PRD 引用的承重命令（docker compose / migration / verify 等），缺一条 = 1 个缺口 |
| `concerns` | 与你 audit 出的妥协清单数差；差 ≥3 → NEEDS_REWORK |
| `acceptance_coverage[S-NNN]` | 每个 slice 至少 1 条独立证据（代码位置 / 测试位置 / 命令输出）；不能只回引 acceptance 文本本身 |
...
2. **三问对账** — 对每条原子条目：
   ...
   - **b. 验证存在？** 找对应测试断言（unit / integration / E2E 任一层）；找不到 → 缺口。
   - **c. 验证够？** PRD 写「X 被阻止 / 拒绝 / 限制」必须有 negative-path 断言（数据库 unique / NOT NULL 约束不算业务层验证）；PRD 引用的承重命令必须出现在 `commands` 字段。缺一条 → 缺口。
3. **数 concerns 差距** — 你 audit 出的妥协清单 vs `impl_result.concerns` 长度差。
4. **填对账表 + 选 verdict** — 见报告模板与决策树。
...
2. 任一 slice 完成标志在 diff 中找不到对应实现 → **NEEDS_REWORK**
3. 任一 slice 完成标志没有对应测试断言（且非纯展示） → **NEEDS_REWORK**
4. PRD 引用的承重命令未在 `impl_result.commands` 字段里 → **NEEDS_REWORK**
5. 你 audit 出 impl 漏掉的 concerns ≥3 条 → **NEEDS_REWORK**
6. 漏 concerns 1-2 条且无其它缺口 → **APPROVED_WITH_NOTES**（note 里追加遗漏 concern）
7. 全部对账通过且 0 漏 → **APPROVED**
...
| Slice | 完成标志（从 PRD 拷） | 实现位置 | 测试断言 | 缺口 |
|---|---|---|---|---|
| S-001 | <PRD 原文> | `<file>:<line>` | `<test file>:<line>` | 无 / <具体缺口> |
