...
- 只通过 `sdd-arbor record-review` 追加 review log / 更新 lifecycle；不手写 `task.json` 或 context JSONL。
...
- 使用 `sdd-arbor record-review <package> --task T-xxx --state <verdict> --summary "..." --evidence "..." --note "..."` 更新 review state、顶层聚合状态和 phase history，并追加 `review.md` entry。
