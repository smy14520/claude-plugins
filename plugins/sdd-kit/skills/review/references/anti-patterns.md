...
## 承重 required_check 没有 run-check evidence

PRD / slice Verification 明确提到 `docker compose run verify` / `prisma migrate deploy` / `npm run e2e:offline` 等承重命令，但对应 required_check 没有 `source=run-check`、`exit_code=0` 与输出文件。impl 自己声明跑了但没留 check evidence → 算缺口。

> 例：PRD S-006「提供 docker-compose 验证路径」+ 关键约束「不依赖第三方」，但 checks 只有 `npm run` 系列，没有任何 docker required_check 的 passed run-check。
