---
status: open | ready-for-brainstorm | closed
topic: <topic-kebab-case>
updated: YYYY-MM-DD
downstream: <brainstorm-name | direct-answer | wiki-ingest>
---


# <topic>

## 研究问题

<一句话描述当前正在探索的问题。允许是暂定性提法，而非最终定义。>

## How might we

<可选：将当前需求改写成机会问题，例如“如何帮助 <用户/执行者> 在 <约束> 下达成 <结果>？”>

## 当前理解

<当前最可能的理解是什么？需求更像在解决用户痛点、工作流问题、技术约束、采用阻力还是方案假设？哪些地方仍然是 provisional 的？>

## 候选理解

- **H1**: <一种可能理解> — 哪些来源让它看起来合理？仍缺什么信息？
- **H2**: <另一种可能理解> — 哪些来源让它看起来合理？仍缺什么信息？

## 待澄清点

- <需要 brainstorm 继续追问或定稿前澄清的问题>
- <需要更多 source 才能判断的问题>

## 当前范围 / 暂不纳入

### 当前范围

- <当前优先探索的子问题 1>
- <当前优先探索的子问题 2>
- <当前优先探索的子问题 3>

### 暂不纳入

- <当前明确不纳入研究的内容 1> — <为什么现在不纳入>
- <当前明确不纳入的内容 2> — <为什么现在不纳入>

## 意图锚定

- **Decision type**: 选型 / 估工 / 理解现状 / 评估可行性
- **Success criteria**: <什么样的发现算“有用”？1-3 条具体判断标准>
- **Downstream**: <brainstorm-name / 直接回答 / wiki 摄取>

## 主题导航

- [`notes/<subtopic-a>.md`](notes/<subtopic-a>.md) — <一句话说明它讲什么>
- [`notes/<subtopic-b>.md`](notes/<subtopic-b>.md) — <一句话说明>

## 关键来源

- [`raw/<source-a>.md`](raw/<source-a>.md) — <为什么关键>
- [`raw/<source-b>.md`](raw/<source-b>.md) — <为什么关键>

## 仍未解决的问题

- <尚未解决、且会影响后续 brainstorm 或继续 research 的问题 1>
- <问题 2>

## 当前是否适合进入 brainstorm

<open / ready-for-brainstorm / closed 的当前判断。若 ready-for-brainstorm，说明为什么；若还不够，说明主要缺口。ready-for-brainstorm 不表示 implementation framing 已冻结；package scope、Slices、技术栈、架构、repo baseline 仍由 brainstorm 检查和澄清。>

## 下一步（由用户决定）

- 继续 research：补资料 / 继续澄清 / 新增 notes
- 进入 `brainstorm`：如果当前理解已经足够收敛
- 运行 `wiki`：仅在你明确要沉淀长期知识时
