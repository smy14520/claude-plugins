...
4. 必要时用 `AskUserQuestion` 提 1-3 个高杠杆问题；每题给 2-4 个选项，推荐项放第一
5. 更新 `index.md`
6. 在 `log.md` 追加记录
7. 输出 framing；不要把需要用户决策的问题只写成文字列表
...
若足够 → `status: ready-for-brainstorm`（只表示足够进入 brainstorm；implementation framing、package scope 与 Slices 仍由 brainstorm 检查）
