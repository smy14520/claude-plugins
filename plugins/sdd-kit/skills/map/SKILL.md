...
`parallel` 会内部执行 `map-check` / `map-plan-agents`，由主会话作为 lead 创建 runtime Agent Team worker pool，并按 `execution_ready` / `prep_ready` 启动最多 3 个 worktree-isolated package worker。Parallel uses Team messages for coordination and local checkpoint commits for code synchronization.
...
9. 跨域 contract 是一等公民。
10. Map 不隐式推进到 task/impl；Agent Team 自主推进交给显式 `parallel`。
