...
`map` 是大项目可选层：当一个上位主题自然拆成多个 bounded task packages 时，用 `.arbor/maps/<project>.md` 维护子域、跨域契约、依赖和下一步导航。small / medium case 不需要强行创建 map。

每阶段独立可用，文档之间**无强制跳转**。`research` 负责发散与澄清；`map` 负责大项目的多子域导航；`brainstorm` 负责把当前 change 收敛为 package-local PRD/context artifact；`task` 负责执行冻结与拆解；`impl` 负责消费 task-local context 交付代码；`review` 负责独立语义审计。`[[wikilink]]` 仅作为可选导航线索。

| 阶段 | 职责 | 产物位置 |
|------|------|---------|
| research | index-first 的需求探索工作区：发散、提问、收集资料、带来源地解释事实、逐步收敛理解，并通过 `index.md` 对外提供统一入口 | `.arbor/research/<topic>/` |
| map | 大项目可选导航层：维护子域、bounded task packages、跨域契约、shared capability、依赖关系与当前 blocker | `.arbor/maps/<project>.md` |
| brainstorm | 可直接供 task 拆解的 package-local PRD/context 文档：背景、范围、场景、交付物、切片建议、风险、来源 | `.arbor/tasks/<name>/prd.md` |
| task | 执行冻结 + 任务拆解：milestone / child task / DAG / task-local context / sources | `.arbor/tasks/<name>/task.md` + `task.json` + `context/*.jsonl` |
| impl | 按 task 执行代码实现 + 运行自己的 acceptance（SelfCheck） | 代码本身 + `.arbor/tasks/<name>/task.json` |
| review | 对照 PRD + task + diff + wiki 做独立语义审计（4 态: APPROVED / APPROVED_WITH_NOTES / NEEDS_REWORK / BRAINSTORM_DRIFT） | `.arbor/tasks/<name>/review.md` |

**Task lifecycle 约定**：`task.json` 是 task package 的唯一生命周期状态源；`prd.md` 是 package-local PRD/context artifact；`task.md` 是稳定任务定义，impl/review 不修改；`review.md` 是追加式语义审计日志，不作为当前状态源；`status.md` 已废弃，新的 task package 不得创建。
...
python3 plugins/sdd-kit/tools/arbor.py set-prd-status <name> --status ready-for-task --actor brainstorm --note "prd ready"
...
python3 plugins/sdd-kit/tools/arbor.py show <name>
```

脚本只做机械事务：创建目录、初始化模板、维护状态、追加 JSONL、校验 schema。它不判断需求范围、不写 PRD、不拆任务语义、不决定 review 是否通过，也不会把 `task.json.tasks[]` 自动同步进 `task.md`；task skill 必须写实 Markdown 任务定义。
...
- 它是**单次 feature / change 的 package-local PRD/context artifact**
- 它首先服务于 `task` 的直接拆解，而不是追求“看起来很严谨”的 contract 模板
- 新写入位置是 `.arbor/tasks/<name>/prd.md`

Legacy note：旧 `.arbor/brainstorms/<name>.md` 只作为 fallback input。新流程不再创建该路径。
...
- [x] map skill（大项目 domain/project map + bounded task package registry + cross-domain contracts）
- [x] brainstorm skill（package-local PRD/context artifact + 来源追踪 + 可拆解性优先）
- [x] task skill（执行冻结 + milestone / child task + DAG + context JSONL / sources）
...
| 第二阶段产物 | design/spec 并存，边界较重 | `brainstorm`：以 task 可拆解性为核心的 package-local PRD/context artifact |
