...
sdd-arbor finish-worker <initiative> <package> --assignment-id <id> --from-worktree ../arbor-worktrees/<project-name>/<initiative>/<package> --review-state ready_for_review --changed-artifact prd --json
sdd-arbor upsert-contract <initiative> --consumer <package-b> --producer <package-a> --request "需要 producer 提供的稳定 contract" --status open --json
...
sdd-arbor claim-package <package> --owner <actor> --branch arbor/<initiative>/<package> --worktree ../arbor-worktrees/<project-name>/<initiative>/<package>
sdd-arbor import-package-artifacts <package> --from-worktree ../arbor-worktrees/<project-name>/<initiative>/<package>
...
sdd-arbor record-agent <package> --role review --agent <agent-name> --status passed --summary "..."
sdd-arbor validate <package>
sdd-arbor show <package>
```

脚本只做机械事务：创建目录、初始化模板、维护状态、追加 JSONL、校验 schema、记录 package-level execution metadata、聚合 map execution_ready/prep_ready/blocked 状态、选择动态 parallel lane、生成 worker assignment/context packet、自愈 missing dispatch / stale blocker / worker finish / contract upsert，以及用 `import-package-artifacts` 从 worker worktree 回收 package artifacts（不覆盖 `task.json` control state）。它不判断需求范围、不写 PRD、不拆任务语义、不决定 review 是否通过、不创建 worktree/branch/PR、不启动 agent，也不会把 `task.json.tasks[]` 自动同步进 `task.md`；task skill 必须写实 Markdown 任务定义。
...
- [x] parallel skill（动态 lane scheduler + Agent Team worker pool + self-healing helpers + EnterWorktree gate）
...
- [x] deterministic helper（`sdd-arbor`：create / validate / scheduler / self-healing / context / wiki retrieval / lifecycle state）
...
自然语言（显式点名，推荐）:
  ...
  "并行推进 <initiative>"              # 指定 initiative，默认最多 5 个 worker，自主推进 runnable packages
...
`parallel` 是自动动态吞吐优化入口；用户不需要每次手写 `map-check` / `parallel-schedule` / self-healing helper，也不需要通过 `map --parallel` 间接触发。执行 `/sdd-kit:parallel` 表示授权主会话创建 Agent Team runtime，在 serial critical-path、parallel execution、parallel prep、serial integration 间自动切换；lane switch 只汇报，true blocker 才停。如果要人工审计每个阶段，直接调用 `/sdd-kit:brainstorm` / `/sdd-kit:task` / `/sdd-kit:impl` / `/sdd-kit:review`。插件 skill 的 slash command 是命名空间形式 `/sdd-kit:parallel`；裸 `/parallel` 只有在运行时额外提供别名时才成立。
...
| Agent | 9 agents | 无内置固定 agent；`parallel` 由主会话 lead 动态调度 single worker / Team worker pool / serial integration worker，worker 在独立 branch/worktree 的 package boundary 内推进 pipeline；package 可记录 branch/worktree/PR/agent validation metadata，review 仍建议在独立上下文里跑 |
