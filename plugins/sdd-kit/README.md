...
```text
research? → brainstorm
              ├─ obvious small: .arbor/tasks/<package>/prd.md → task → impl → review
              └─ needs boundary routing: clarified framing → map
                                                              ├─ single package → task → impl → review
                                                              └─ split packages → package-local brainstorm → task → impl → review
                                                                                 ↑
                                                                                 └─ 用户主动 ingest → wiki（持久）
```

`map` 是可选的 boundary routing / package graph 层：对 clarified framing 判断 single package、split packages，或退回 brainstorm。只有 split 时才用 `.arbor/maps/<initiative>/map.md` / `map.json` 维护 package graph、execution waves、跨 package 契约、依赖和下一步导航，并 materialize child package stubs。small / medium case 不需要强行创建 map，但用户想多一道边界判断时可以显式使用。

每阶段独立可用，文档之间**无强制跳转**。

可选：sdd-kit 提供自然语言触发的 `team-auto` skill 作为会话层 Agent Team 协作能力，不是 workflow 阶段，也不是 parallel runtime。用户在任意阶段说“用 Team Auto / 开多 agent / 双推 / review panel”时，当前会话会先给出 2-4 个阵型选项（如辩论会、双推、Role-lane Parallel、Shadow Review、Review Panel）并推荐一个，由用户选择后再启动；用户明确授权时才直接开 Team。详见 `skills/team-auto/`。

- `research` 负责发散与澄清。
- `brainstorm` 必须先完成需求澄清与整体 design framing：明显小需求可直接形成 single executable package PRD；边界不自然或用户希望再判断时，输出 clarified framing 交给 `map`。
- `map` 负责 boundary routing：可确认 single package、拆出 package graph，或退回 brainstorm；split 后维护 execution waves / contracts / blocker navigation，并 materialize child package stubs；它不是执行器，不自动派发实现。
...
| brainstorm | 需求澄清 + design framing：明显小需求写 single package PRD；边界不自然或用户希望再判断时，只输出 clarified framing / map handoff，不创建 child stubs | `.arbor/tasks/<package>/prd.md` 或 clarified framing / map handoff |
| map | Boundary routing / package graph：对 clarified framing 判断 single package、split packages 或退回 brainstorm；split 后维护 execution waves、跨 package 契约、依赖与导航，并 materialize child package stubs | `.arbor/maps/<initiative>/map.md` + `.arbor/maps/<initiative>/map.json` + `.arbor/tasks/<package>/` stubs，或 `.arbor/tasks/<package>/prd.md` |
...
- Brainstorm 先完成 clarified framing；明显 small 可直接记录 `fits_package`。
- Map 对 clarified framing 做 boundary routing：single package 记录 `fits_package`；split packages 确认 package graph 后，应立即 materialize child package stubs：`.arbor/tasks/<package>/`，并记录 `package_sizing=split_applied`。
...
- Brainstorm/map 必须先完成 package boundary sizing：brainstorm 可处理 obvious single package 的 `fits_package`，map 可确认 single package 或 split 后 child package 的 `split_applied`；task 只验证 sizing 状态并阻止 `unchecked` / `split_recommended` 进入 T-xxx。
...
sdd-arbor set-package-sizing <package> --status fits_package --actor map --phase map --decision "map 判断当前 framing 适合 single package"
...
- 明显小需求可写入 single executable package PRD：`.arbor/tasks/<package>/prd.md`
- 边界不自然或用户希望再判断时，只输出 clarified framing / map handoff；由 `map` 判断 single package、split packages 或回 brainstorm
...
- [x] map skill（boundary routing + package graph + execution waves + cross-package contracts + serial readiness navigation）
- [x] brainstorm skill（需求澄清 / design framing + obvious single package PRD / map handoff + 来源追踪）
...
自然语言（显式点名，推荐）:
  ...
  "这个 impl 用 Team Auto，先给我几个阵型选项"   # 触发 team-auto skill
...
`map` 可用于 clarified framing 的边界判断；`map-check` 只给出 split initiative 的串行下一步和 blocker。如果要推进实现，直接调用 `/sdd-kit:brainstorm` / `/sdd-kit:task` / `/sdd-kit:impl` / `/sdd-kit:review` 处理对应 package。
...
| 第二阶段产物 | design/spec 并存，边界较重 | `brainstorm`：先做需求澄清 / design framing；明显小需求产出 executable package PRD，边界不自然时交给 map 做 boundary routing |
