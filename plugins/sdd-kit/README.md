...
research? → brainstorm
              ...
              └─ large: clarified framing → map → package-local brainstorm → task → impl → review
                                               │          ↑
                                               └─ parallel?
   ↖                                                        │
   │                                                        └─ 用户主动 ingest → wiki（持久）
...
`map` 是大项目可选统筹层：当一个上位主题自然拆成多个 executable task packages 时，用 `.arbor/maps/<initiative>/map.md` 维护人类可读的 package graph、execution waves、跨 package 契约、依赖、parallel policy 和下一步导航，用 `.arbor/maps/<initiative>/map.json` 维护机器可读状态，并立即 materialize child package stubs：`.arbor/tasks/<package>/`。small / medium case 不需要强行创建 map。
...
- `map` 负责大项目 package graph / execution waves / parallel policy，并 materialize child package stubs。
- `parallel` 是 Trellis-like 一等统筹入口，由主会话作为 lead 创建 Agent Team worker pool，按 `execution_ready` / `prep_ready` 滚动分发最多 3 个 worktree-isolated package workers。
...
| parallel | Trellis-like 一等统筹入口：主会话作为 lead 自动 map-check、生成 execution_ready / prep_ready worker assignment/context、创建 runtime Agent Team worker pool、滚动分发最多 3 个 worktree-isolated package worker，并在 package boundary / assignment limit 内自主推进 brainstorm/task/impl/review；blocked/decision-needed 回报用户 | `.arbor/maps/<initiative>/context/agent-assignments.jsonl` + `.arbor/tasks/<package>/context/worker-dispatch.md` + worker team sessions/worktrees |
...
- `.arbor/tasks/<package>/` 只表示 executable package execution boundary，可记录一个 branch、一个 worktree、一个 PR 和显式 agent validation metadata。
...
- `parallel` skill 是用户级一等入口，内部执行 `map-check` / `map-plan-agents`，只把依赖已 reviewed/completed/merged 的 package 标为 ready，创建 runtime Agent Team，并分发 worktree-isolated package workers；worker 在自己的 package boundary / branch / worktree 内按 `task.json.next_action` 自主推进，遇到不确定、越界、阻塞或外部副作用时回报主调度器。
...
python3 plugins/sdd-kit/tools/arbor.py set-package-sizing <package> --status split_applied --actor map --phase map --decision "package extracted from .arbor/maps/<initiative>/map.md"
python3 plugins/sdd-kit/tools/arbor.py set-prd-status <package> --status ready-for-task --actor brainstorm --note "prd ready"
python3 plugins/sdd-kit/tools/arbor.py add-amendment <package> --title "Refund rule correction" --wrong "refund behavior missing" --correct "full refund revokes entitlement" --affects-task T-003 --actor brainstorm
...
脚本只做机械事务：创建目录、初始化模板、维护状态、追加 JSONL、校验 schema、记录 package-level execution metadata、聚合 map execution_ready/prep_ready/blocked 状态、生成主会话 lead + Agent Team worker pool 的 assignment/context packet。它不判断需求范围、不写 PRD、不拆任务语义、不决定 review 是否通过、不创建 worktree/branch/PR、不启动 agent，也不会把 `task.json.tasks[]` 自动同步进 `task.md`；task skill 必须写实 Markdown 任务定义。
...
- [x] parallel skill（Trellis-like 一等统筹入口 + 主会话 lead + Agent Team rolling worker pool + worktree isolation + context injection packet）
...
自然语言（显式点名，推荐）:
  ...
  "用 parallel skill"                 # 自动选择唯一 map，check + plan + Agent Team worker pool
  "并行推进 <initiative>"              # 指定 initiative，默认最多 3 个 worker，自主推进 runnable packages
...
`parallel` 是 Trellis-like 一等入口；用户不需要每次手写 `map-check` / `map-plan-agents`，也不需要通过 `map --parallel` 间接触发。执行 `/sdd-kit:parallel` 表示授权主会话创建 Agent Team worker pool、自主推进 execution_ready / prep_ready package，并用 worktree/branch 隔离 package diff；如果要人工审计每个阶段，直接调用 `/sdd-kit:brainstorm` / `/sdd-kit:task` / `/sdd-kit:impl` / `/sdd-kit:review`。插件 skill 的 slash command 是命名空间形式 `/sdd-kit:parallel`；裸 `/parallel` 只有在运行时额外提供别名时才成立。
...
| Agent | 9 agents | 无内置固定 agent；`parallel` 由主会话 lead 按 execution_ready/prep_ready package 创建最多 3 个 Agent Team worker，worker 在独立 branch/worktree 的 package boundary 内推进 pipeline；package 可记录 branch/worktree/PR/agent validation metadata，review 仍建议在独立上下文里跑 |
