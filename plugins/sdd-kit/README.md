...
- [ ] S-001: 项目基建 — 脚手架、配置、健康检查、dev 启动命令
- [ ] S-002: 认证与权限 — 登录、角色、权限边界
- [ ] S-003: 核心业务闭环 — 关键 API / 页面 / 数据状态
- [ ] S-004: 验收自检 — 核心路径、边界路径、回归检查
```

Slice 是 PRD-local 的执行切片，不是独立 PRD，也不会 materialize 成子 package。Impl 只更新 slice 进度标记：

- `[ ]` 未开始
- `[-]` 部分完成（附简短备注）
- `[x]` 完成

Package 是需求 / 实现 / 评审 / 回滚边界；PRD 是需求、Technical Framing、Acceptance Criteria 和 Slices 的 source of truth。代码状态是实际进度事实，slice 标记是断点续作提示。
