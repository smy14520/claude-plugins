"""Unit tests for the pure-function helpers in the scenario eval framework.
...
- `slice_progress`: slice counter and partial-progress indicator.
"""
...
from framework.checks import (  # noqa: E402
...
from framework.phases import (  # noqa: E402
    _is_task_state_ready,
    _derive_impl_started,
    _has_brainstorm_ready_text_marker,
    _derive_review_recorded as phases_derive_review_recorded,
    review_prompt,
    direct_prompt,
    response_timeout_for_phase,
)
from framework.runner import extract_result_metrics  # noqa: E402
...
from framework.ai_user import fallback_answer  # noqa: E402
...
class DetectBlockingOpenQuestionTests(unittest.TestCase):
    def test_returns_false_when_no_open_questions_section(self):
        ...
        self.assertFalse(_detect_blocking_open_question(prd))
    ...
    def test_returns_true_for_unresolved_blocking_colon_marker(self):
        prd = "# Demo\n\n## Open Questions\n\n- Blocking: 需要确认支付服务商\n"
        self.assertTrue(_detect_blocking_open_question(prd))
...
class DeriveImplStartedTests(unittest.TestCase):
    ...
    def test_todo_greenfield_small_run_is_reported_as_started(self):
        ...
        self.assertTrue(_derive_impl_started(task_state))
...
class SliceProgressTests(unittest.TestCase):
    ...
    def test_counts_done_and_in_progress(self):
        ...
        progress = slice_progress(task_state)
    ...
    def test_impl_in_progress_false_when_impl_result_recorded(self):
        ...
        self.assertFalse(slice_progress(task_state)["impl_in_progress"])
...
class BrainstormReadyTextMarkerTests(unittest.TestCase):
    def test_returns_true_for_finalized_marker(self):
        self.assertTrue(_has_brainstorm_ready_text_marker("finalized: demo"))

    def test_returns_true_for_ready_package_written(self):
        self.assertTrue(
            _has_brainstorm_ready_text_marker("已定稿并写入 ready package")
        )
    ...
    def test_returns_false_for_confirmation_request_with_next_impl_hint(self):
        ...
        self.assertFalse(_has_brainstorm_ready_text_marker(response))


class ResponseTimeoutForPhaseTests(unittest.TestCase):
    def test_defaults_impl_timeout_longer_than_brainstorm(self):
        self.assertEqual(response_timeout_for_phase("brainstorm"), 900)
        self.assertEqual(response_timeout_for_phase("impl"), 2400)
    ...
    def test_brainstorm_uses_legacy_response_timeout_fallback(self):
        with mock.patch.dict(
            ...
            self.assertEqual(response_timeout_for_phase("brainstorm"), 240)
            self.assertEqual(response_timeout_for_phase("impl"), 2400)
    ...
    def test_phase_specific_env_overrides(self):
        with mock.patch.dict(
            ...
            self.assertEqual(response_timeout_for_phase("brainstorm"), 300)
            self.assertEqual(response_timeout_for_phase("impl"), 1200)
...
class ReviewPromptAndStateTests(unittest.TestCase):
    def test_review_prompt_uses_package_name_from_prd_path(self):
        ...
        self.assertEqual(review_prompt(prd_path), "用 review 审计这个 package PRD：demo-package")

    def test_review_prompt_falls_back_to_current_package(self):
        self.assertEqual(review_prompt(None), "用 review 审计当前 package PRD")

    def test_review_recorded_false_for_missing_state(self):
        self.assertFalse(derive_review_recorded(None))
        self.assertFalse(derive_review_recorded({}))

    def test_review_recorded_true_when_review_result_exists(self):
        self.assertTrue(derive_review_recorded({"review_result": {"state": "APPROVED"}}))

    def test_review_state_returns_recorded_state(self):
        self.assertEqual(review_state({"review_result": {"state": "NEEDS_REWORK"}}), "NEEDS_REWORK")
        self.assertIsNone(review_state({"review_result": None}))


class DirectPromptTests(unittest.TestCase):
    def test_direct_prompt_forbids_sdd_workflow_and_followups(self):
        prompt = direct_prompt("做一个本地社区活动管理工具")
...
class MetricsAggregationTests(unittest.TestCase):
    ...
    def test_extract_result_metrics_captures_all_fields(self):
        metrics = extract_result_metrics(self.FakeResult())
    ...
    def test_add_phase_metrics_accumulates_cost_and_duration(self):
        bucket = empty_metrics()
        add_phase_metrics(bucket, "brainstorm", {"duration_ms": 100, "total_cost_usd": 0.1})
        add_phase_metrics(bucket, "brainstorm", {"duration_ms": 50, "total_cost_usd": 0.15})
        self.assertEqual(bucket["duration_ms"], 150)
        self.assertAlmostEqual(bucket["total_cost_usd"], 0.25)
        self.assertEqual(bucket["by_phase"]["brainstorm"]["duration_ms"], 150)

    def test_finalize_run_metrics_computes_combined_totals(self):
        metrics = new_run_metrics()
        add_phase_metrics(metrics["tested_agent"], "brainstorm", {"duration_ms": 100, "total_cost_usd": 0.3})
        add_phase_metrics(metrics["ai_user"], "brainstorm", {"duration_ms": 20, "total_cost_usd": 0.2})
        final = finalize_run_metrics(metrics)
...
class MatrixAggregationTests(unittest.TestCase):
    def test_aggregate_matrix_counts_diff_hashes_and_handles_eval_scores(self):
        summaries = [
            {
                "group": "A-direct",
                "scenario_name": "test",
                "scenario_profile": "greenfield-small",
                "verdict": "needs_review",
                "run_error": None,
                "diff": {"sha256": "a", "changed_files": 2},
                "metrics": {"harness_total": {"tested_agent_total_cost_usd": 0.1, "tested_agent_duration_ms": 100}},
                "evaluation": {"total_score": 3.5},
            },
            {
                "group": "A-direct",
                "scenario_name": "test",
        ...
        self.assertEqual(aggregate["groups"]["A-direct"]["eval_scores"]["count"], 2)
        self.assertAlmostEqual(aggregate["groups"]["A-direct"]["eval_scores"]["mean"], 3.75)
        self.assertEqual(aggregate["groups"]["B-sdd"]["eval_scores"]["count"], 1)
...
class QualityChecksSmokeTests(unittest.TestCase):
    ...
    def test_verification_section_counts_as_acceptance_signal(self):
        prd = "# Demo\n\n## 验证重点\n\n- 主路径跑通\n"
        result = quality_checks(prd, turns=1, task_state=None)
    ...
    def test_blocking_check_uses_section_scoped_detector(self):
        ...
        result = quality_checks(prd, turns=5, task_state=None)
        self.assertFalse(
