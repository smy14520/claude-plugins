...
class ArborCliTests(unittest.TestCase):
    ...
    def test_create_map_writes_directory_artifacts(self):
        ...
        self.assertEqual(data["orchestration"]["strategy"], "autonomous-package-pipeline")
    ...
    def test_create_map_upgrades_old_orchestration_strategy(self):
        ...
        self.assertEqual(upgraded["orchestration"]["strategy"], "autonomous-package-pipeline")
    ...
    def test_map_plan_agents_generates_context_for_ready_packages(self):
        ...
        plan = arbor.map_plan_agents(self.root, "big-init", 2, "map", NOW)
        self.assertEqual([item["package"] for item in plan["assignments"]], ["big-core", "big-ledger"])
        self.assertIn(".arbor/maps/big-init/map.json", plan["assignments"][0]["context_files"])
        self.assertIn("Act as a sdd-kit package dispatch worker", plan["assignments"][0]["worker_prompt"])
        self.assertIn("keep advancing brainstorm/task/impl/review", plan["assignments"][0]["worker_prompt"])
