---
name: handoff
description: "把当前上下文压缩蒸馏为标准交接文档，沉淀至 .forge 任务目录。既支持用户随时输入 /handoff 交接会话，又被编排流用于各阶段边界的上下文隔离交接。"
---

# Handoff — 上下文压缩与状态交接

把当前会话中高噪音的执行细节、探索草稿与终端输出过滤掉，蒸馏出一份**高信噪比、具备无损续接能力**的标准 Markdown 交接文档，让接棒者（新会话或独立子 Agent）在全新上下文里无需翻阅历史记录即可立即开工。

## 核心法则：不可推导性与自包含续接

1. **不可推导性准则（Non-Derivability Criterion）**：
   - **可推导事实不冗余拷贝**：引用路径、符号名和 spec 位置，不全文拷贝代码与日志；
   - **不可推导事实交代清楚**：凡代码和 Git 读不出的隐性决策、被证实的死胡同（试过但行不通的方案及原因）、外部依赖 Gotcha、半成品的微妙 Seams，交代清楚技术因果。
2. **自包含无损续接（Self-Contained Continuation）**：
   - 唯一的合格标准：一个完全没看过上轮对话的工程师/Agent，仅凭工作树代码 + `spec.md` + 本交接文档，能否不猜、不问、不重复走弯路地立即开工。
   - 简单改动 3 句话 50 字即止，复杂并发/架构改动写透上千字亦属合理，不设机械字数限制。

## 适用场景

1. **编排流阶段流转（Model-invoked）**：在 `/develop` 等编排中，每当对齐（Align）、探索（Explore）、实现（Implement）完成时，编排器或执行者调用本技能落盘交接。
2. **开发者主动暂停或交接（User-invoked）**：开发者输入 `/handoff [说明]`，用于长会话重置 context、换终端或交接给队友。

## 执行流程

### 1. 确定目标路径与序号
- 定位目标任务目录 `.forge/tasks/<slug>/`；
- 读取任务当前 `state.json` 获取当前 `phase`；
- 扫描 `handoffs/` 目录中已有文件数量，序号递增：格式为 `handoffs/{NN}-{phase_lower}.md`（如 `01-align.md`, `02-explore.md`, `03-impl.md`）。

### 2. 蒸馏五段式标准内容（基于 templates/handoff.md）

以高密度、事实化语言撰写以下 5 个小节：

1. **Settled Decisions (已锁定的决策)**：
   - 拍定的技术选型、架构方向、明确否决的备选路径及理由；
   - 形成不可推翻的阶段共识。
2. **Agreed Seams & Contracts (Seams 契约)**：
   - 本阶段明确或演化的核心 Seams（对外公开的可观测行为契约与验收依据）；
   - TDD 与审查的唯一客观锚点。
3. **Discovered Gotchas (隐性事实与 Gotcha)**：
   - 探索和编码中验证出的真实事实：依赖怪癖、环境陷阱、性能拐点。
4. **Touchpoints (改动文件与边界)**：
   - 已触碰或下一步明确要修改的核心文件及其职责。
5. **Next Objective (下阶段核心交付目标)**：
   - 交付给下一个接棒者（人类或下一个子 Agent）的具体动作与检验标准。

### 3. 落盘与状态更新
- 使用 `Write` 工具将文档写入 `.forge/tasks/<slug>/handoffs/{NN}-{phase_lower}.md`；
- 使用 `Edit` 工具将 `state.json` 中的 `phase` 更新为下一个阶段（如从 `ALIGN` 更新为 `IMPLEMENT`）；
- 向调用方返回交接文档路径与一句话核心成果摘要。
