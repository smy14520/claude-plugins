...
3. **物理上下文防爆（Handoff Chaining）**：每个阶段结束将高信噪比交接文档写入 `handoffs/`，下一阶段子 Agent 只读交接文档，中间高噪音上下文就地释放；
...
1. **派发【独立全新上下文】的 `forge-impl` 子 Agent**：
   ...
   - **严格控制输入**：只传入 task_slug、`spec.md` 路径及最新 `handoff.md` 路径，绝不传入阶段 1 的冗长问答历史；
2. 等待子 Agent 执行完毕并回收交付成果；
3. 子 Agent 会话销毁，高噪音执行垃圾物理释放；
...
### Phase 5: 知识淬火、成果呈递与交还控制权 (Presentation & Quenching)

主会话执行收尾淬火，并向人类开发者呈递高浓度汇总：

1. **知识淬火与 Spec 冻结（Quenching & Promotion）**：
   - **Spec 冻结（Delta 归档）**：`.forge/tasks/<slug>/spec.md` 伴随任务完成立即冻结归档，封入任务历史；未来任何任务绝不将其当作现时标准加载（终结 PRD 坟场）；
   - **知识晋升（State 沉淀）**：若本任务产生了不可逆架构决策、跨系统联动拓扑、新领域实体或暗坑，使用 `Write` 工具将决策沉淀至 `.forge/wiki/decision/` 或提议写入 `.claude/rules/`；
2. **背书钢印呈递**：
   - 核心 Seams 行为契约兑现情况；
   - 全量防回归测试结果（或 `perceive` 物理证据截图/日志）；
   - 双轴审查评级与安全核对清单；
3. **交付说明与控制权交还**：
   - 呈现新功能的核心亮点与改动文件清单；
