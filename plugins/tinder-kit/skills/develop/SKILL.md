...
- 自动执行 `grill-with-docs`：
  - 驱动决策树追问（grilling）与按需领域建模（domain-modeling，仅当出现概念分歧时记录词汇表，发生不可逆分叉时才记 ADR）；
  - 查证代码库事实，不问自己能查到的信息；
  - **防呆保真铁律（Poka-Yoke）**：严禁私自裁剪用户在原始需求中显式提及的任何特性！若建议将某项用户明确提出的功能延后或排除，必须作为【单独的聚焦问题】向人类申请裁决，绝不允许隐式混入批量默认值清单偷渡；
...
- **🛑 人类把关点 1**：在终端呈递方案核心接缝与边界（附带用户初始需求逐项对账核对表）：
  - **单会话需求**（无需跨会话拆工单）：确认后直接在当前上下文进入实现，免去写 spec.md 的中转开销；
