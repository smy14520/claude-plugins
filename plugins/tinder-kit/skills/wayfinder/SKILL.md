...
## Not yet specified (战争迷雾区)

<!-- 参见“战争迷雾”节：在 Scope 范围内但当前还说不清楚、无法立票的朦胧区域；随着前沿推进逐步升级为票 -->
- 迷雾区域描述 1
...
- **Prototype（HITL）**：通过粗糙、自包含的原型提高讨论置信度（调用 `prototype` 技能在 `.forge/prototypes/` 探索），核心问题是“跑起来手感如何”或“外观长什么样”时使用。
...
地图是**有意不完整**的：不要描绘你还看不见的东西。Tickets 之外是迷雾：那些你能隐约感觉到以后会来的决策，但它们悬在尚未解决的前置问题之上，暂时还无法钉住。

**Fog or Ticket？（是迷雾还是立票？）判据绝对分明**：
- **可以立票（Ticket）**：如果你**现在就能把问题精确表述清楚**（哪怕它被前置票 Blocked 暂时不能做），立刻建票！
- **留在迷雾（Not yet specified）**：如果你现在还说不清楚，坚决不要把迷雾预先切成小票！迷雾比票粗大得多，当工期推移到该区域时，一片迷雾可能分化出 3 张票，也可能一张都没有。
