...
- **Considered Options（被否决的备选方案）**: 技术选型类决策写上；每个备选一行否决理由，并链接证据原处（`.forge/research/` 资料、原型 Verdict、wayfinder 票），证据只引用不复制；
- **Consequences（下游影响）**: 接受了什么代价，或存在反直觉的下游连带影响时记录；
- **Revisit When（重新评估信号）**: 什么信号出现时值得重新评估（如“单文档并发编辑 > 50 人”“托管方案价格降到 X”）。One-way door 不是永不回头，是回头很贵，所以事先写下何时值得回头。
...
如果一个决定很容易回滚改动，跳过它（直接改就行）；如果它很符合常理、显而易见，没人会问为什么；如果没有真正的替代方案，直接改就行，不建 ADR。
