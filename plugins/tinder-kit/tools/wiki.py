#!/usr/bin/env python3
"""forge wiki — 项目本地 `.forge/wiki/` 导航层 CLI。

wiki 是导航层，永不是 source of truth；本 CLI 只做 markdown 页面的索引、搜索、
收集与体检。index.md / log.md 是给人浏览的派生产物，agent 走 collect 分级检索。
...
def wiki_root_path(root: Path, wiki_root: str | None = None) -> Path:
    value = wiki_root.strip() if isinstance(wiki_root, str) and wiki_root.strip() else ".forge/wiki"
    path = Path(value)
    if path.is_absolute():
        return path
    return root / path
...
def _write_index_md(directory: Path, pages: list[dict[str, Any]], root: Path | None = None) -> None:
    """生成 wiki 根目录下的 index.md（按 type 分组）并追加 log.md。"""
...
def _append_log_md(directory: Path, pages: list[dict[str, Any]], old_stems: set[str]) -> None:
    """追加 log.md 条目，记录当前页面快照和变更（old_stems = 上次索引的页面集）。"""
...
def _tokens(value: Any) -> set[str]:
    ...
    if not isinstance(value, str):
        return set()
    tokens = {item.casefold() for item in TOKEN_RE.findall(value)}
    # 中文没有空格分词：对连续汉字补充 bigram，让“超时重试”能命中“客服超时”
...
def _hidden_path_parts(relative_path: str) -> list[str]:
    return [part for part in Path(relative_path).parts if part.startswith(".") and part not in (".forge",)]
...
def _file_hits(page: dict[str, Any], files: list[str]) -> list[str]:
    """页面锚点命中的改动文件：精确路径或目录前缀都算。"""
    wanted = [_normalize_file(f) for f in files if f.strip()]
    anchored = {_normalize_file(a["file"]) for a in page.get("anchors", [])}
    hits = []
    for target in wanted:
        if any(a == target or a.startswith(target.rstrip("/") + "/") for a in anchored):
            hits.append(target)
    return hits


def wiki_search(root: Path, query: str | None, wiki_root: str | None = None, limit: int | None = None, files: list[str] | None = None) -> dict[str, Any]:
    query_tokens = _tokens(query or "")
    files = files or []
    if not query_tokens and not files:
        raise WikiError("wiki 检索至少需要 --query 或 --files 其中之一")
    ...
    for page in indexed["pages"]:
        ...
        file_hits = _file_hits(page, files)
        if file_hits:
            matched_fields.append("anchors")
            score += len(file_hits) * FILE_ANCHOR_WEIGHT
        if score:
    ...
    return {"wiki_root": indexed["wiki_root"], "query": query, "files": files, "results": results}


def wiki_collect(root: Path, query: str | None, wiki_root: str | None = None, limit: int = 5, files: list[str] | None = None) -> dict[str, Any]:
    search = wiki_search(root, query, wiki_root, limit, files)
    selected = []
    ...
    return {"wiki_root": search["wiki_root"], "query": query, "files": search["files"], "selected": selected}


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="forge wiki", description="项目本地 .forge/wiki 导航层：index / search / collect / lint")
    parser.add_argument("--root", default=".", help="项目根目录（含 .forge/wiki/）")
    sub = parser.add_subparsers(dest="command", required=True)

    index_parser = sub.add_parser("index", help="为 wiki 页面建立索引")
    index_parser.add_argument("--wiki-root", default=".forge/wiki")
    ...
    search_parser.add_argument("--files", nargs="+", default=[], help="本次要改动的文件或目录，按页面符号锚命中")
    search_parser.add_argument("--wiki-root", default=".forge/wiki")
    ...
    collect_parser.add_argument("--files", nargs="+", default=[], help="本次要改动的文件或目录，按页面符号锚命中")
    collect_parser.add_argument("--wiki-root", default=".forge/wiki")
    ...
    lint_parser.add_argument("--wiki-root", default=".forge/wiki")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.command == "search":
            result = wiki_search(root, args.query, args.wiki_root, args.limit, args.files)
            if json_output:
                print(json.dumps(result, ensure_ascii=False, indent=2))
            else:
        ...
        if args.command == "collect":
            result = wiki_collect(root, args.query, args.wiki_root, args.limit, args.files)
            if json_output:
                print(json.dumps(result, ensure_ascii=False, indent=2))
            else:
