...
- `wiki`: 本地 Markdown 个人知识库 —— 页面、双向链接与标签 (_Avoid_: vault, note, zettelkasten)
...
- `index`: 从页面文件推导、可整体丢弃重建的派生查询数据 (_Avoid_: cache, db, database)
- `link-graph`: 页面间双向链接构成的图及其解析与诊断 (_Avoid_: backlink-graph, 链接网)
- `stdlib`: 运行时仅依赖 Python 标准库的实现约束 (_Avoid_: zero-dependency, nodeps)
