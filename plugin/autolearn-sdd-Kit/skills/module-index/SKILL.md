...
**索引按项目组织**，存储在 `~/.claude/context/modules/<project>/`

```bash
# 在 ~/code/lecturer-api/ 执行
/module-index app/Module/User

# 保存到
→ ~/.claude/context/modules/lecturer-api/User-index.md
```
...
2. 查看 `~/.claude/context/modules/<project>/` 下有哪些索引
3. 根据需求关键词匹配模块名
4. 自动加载匹配的模块索引
...
需求: "修改 User 模块的登录逻辑"
        ↓
匹配: ~/.claude/context/modules/lecturer-api/User-index.md
        ↓
